...
export function getPageFragment(
    ...
    const { addHeader = true, libraryName } = options;
    ...
    const library = libraryName ? createFragment(`library ${libraryName};`) : undefined;

    // Create imports fragment
...
// Helper function to add common imports
export function addImport(fragment: Fragment, importPath: string): Fragment {
    const newImports = new Set(fragment.imports);
    newImports.add(importPath);
...
// Helper function to add multiple imports
export function addImports(fragment: Fragment, imports: string[]): Fragment {
    const newImports = new Set(fragment.imports);
    imports.forEach(imp => newImports.add(imp));
